...
- ✅ **Use Mediator package for request handling** (martinothamar/Mediator)
...
public class GetAllByPageCategoryQueryHandler : IRequestHandler<GetAllByPageCategoryQuery, MyAppResponse<PagedResult<GetAllByPageCategoryDto>>>
{
    ...
    public GetAllByPageCategoryQueryHandler(IApplicationDbContext dbContext, IMapper mapper)
    {
        _dbContext = dbContext;
        _mapper = mapper;
    }
    ...
    public async ValueTask<MyAppResponse<PagedResult<GetAllByPageCategoryDto>>> Handle(GetAllByPageCategoryQuery request, CancellationToken cancellationToken)
    {
        ...
        var mappedCategories = _mapper.Map<List<GetAllByPageCategoryDto>>(categories);
        var pagedResult = new PagedResult<GetAllByPageCategoryDto>(mappedCategories, totalCount, request.PageIndex, request.PageSize);

        return MyAppResponse<PagedResult<GetAllByPageCategoryDto>>.Success(pagedResult);
    }
}
```
...
public async Task Handle_ValidRequest_ShouldCreateCategory()
{
    ...
    // Setup Mapster mapping
    mockMapper.Setup(x => x.Map<Category>(It.IsAny<CreateCategoryCommand>()))
              .Returns(new Category { Id = Guid.NewGuid(), Title = command.Title, IsActive = command.IsActive });
    ...
    mockMapper.Verify(x => x.Map<Category>(It.IsAny<CreateCategoryCommand>()), Times.Once);
}
```
...
- [ ] Integration tests for API endpoints
- [ ] End-to-end tests for critical API journeys
